...
class DenCreateTest(DensTest):
    ...
    def test_implied(self):
        ...
        self.assertExecuted(
            "docker create --hostname {0} --interactive "
            "--label den --name {0} --tty --volume /test/:/src foo".format(
    ...
    def test_name(self):
        ...
        self.assertExecuted(
            "docker create --hostname override --interactive "
        ...
        with self.with_config(image={"name": "configured"}):
            self.invoke.create_den()
            self.assertExecuted(
                "docker create --hostname configured "
        ...
        with self.with_config(image={"name": "configured"}):
            self.invoke.create_den("override")
            self.assertExecuted(
                "docker create --hostname override "
    ...
    def test_command(self):
        ...
        self.assertExecuted(
            "docker create --hostname test --interactive "
    ...
    def test_ports(self):
        ...
        self.assertExecuted(
            ...
            "--tty --volume /test/:/src foo".format(
                self.context.default_name, ports
            )
        )
...
class DenStartTest(DensTest):
    def test_basic(self):
        ...
        self.assertExecuted(
            "docker start --attach --interactive {}".format(
                self.context.default_name
            )
...
class DenStopTest(DensTest):
    def test_basic(self):
        ...
        self.assertExecuted(
            "docker stop --time 1 {}".format(self.context.default_name)
        )
...
class DenDeleteTest(DensTest):
    docker = {"containers": {"list": [{"name": "foo"}, {"name": "bar"}]}}
    ...
    def test_basic(self):
        ...
        self.assertExecuted(
            "docker rm --force {}".format(self.context.default_name)
        )
